...
public interface Waiters {
	...
	default WebElement explicitWaitClickable(WebElement element, WebDriver driver) {
		return new WebDriverWait(driver, 1).until(ExpectedConditions.elementToBeClickable(element));
	}
	...
	default WebElement explicitWaitVisibleByXpath(WebDriver driver, String xpath) {
		return new WebDriverWait(driver, 1).until(ExpectedConditions.visibilityOfElementLocated(By.xpath(xpath)));
	}
	...
	default void waitInvisibility(WebElement element, WebDriver driver) {
		new WebDriverWait(driver, 1).until(ExpectedConditions.invisibilityOf(element));
	}
